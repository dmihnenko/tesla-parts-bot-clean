...
interface SellProductModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (soldPrice: number, buyerInfo: string, selectedModel: string) => void;
...
const SellProductModal: React.FC<SellProductModalProps> = ({
  ...
  React.useEffect(() => {
    if (product) {
      ...
      setBuyerInfoInput('');
      setSelectedModelForSale(Array.isArray(product.model) ? product.model[0] : product.model);
    }
  }, [product]);

  const handleConfirm = () => {
    if (!soldPriceInput || !buyerInfoInput.trim()) return;
    const soldPrice = parseFloat(soldPriceInput);
    if (isNaN(soldPrice) || soldPrice <= 0) {
      alert('Введите корректную цену продажи');
      return;
    }
    onConfirm(soldPrice, buyerInfoInput.trim(), selectedModelForSale);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: isMobile ? '1rem' : '1.5rem' }}>
          <h3 style={{ fontSize: isMobile ? '0.85rem' : '0.9rem', fontWeight: '600', marginBottom: '0.5rem' }}>{product.name}</h3>
          <p style={{ fontSize: isMobile ? '0.8rem' : '0.875rem', color: '#6b7280' }}>
            {Array.isArray(product.model) && product.model.length > 1
              ? `Модель: ${selectedModelForSale} - ${product.category}`
          ...
          <p style={{ fontSize: isMobile ? '0.8rem' : '0.875rem', color: '#374151', marginTop: '0.5rem' }}>
            Оригинальная цена: {usdRate ? `$${Math.round(product.price / usdRate)} / ${product.price}₴` : `${product.price}₴`}
          </p>
        </div>
