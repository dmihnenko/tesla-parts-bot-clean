...
const ModelDetailsModal: React.FC<ModelDetailsModalProps> = ({
  ...
  if (!showModelDetailsModal || !selectedModelForDetails) return null;

  const modelSoldProducts = soldProducts.filter(p =>
    Array.isArray(p.model) ? p.model.includes(selectedModelForDetails) : p.model === selectedModelForDetails
  );
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobile ? '1rem' : '1.5rem' }}>
          <h2 style={{ fontSize: isMobile ? '1.2rem' : '1.5rem', fontWeight: 'bold', color: '#000000' }}>
            Проданные запчасти для {selectedModelForDetails}
          </h2>
          <button onClick={handleCloseModelDetails} style={{
            background: 'none',
            border: 'none',
